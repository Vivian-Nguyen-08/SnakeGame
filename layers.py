...
def fc_backward(grad_out, cache):
    ...
    grad_x, grad_w, grad_b = grad_out @ w.T, x.T @ grad_out, (one_column.T @ grad_out).ravel()
...
def softmax_loss(x, y):
    ...
    N = x.shape[0]
    for i in range(N):
        loss = loss + -np.log(prob[i, y[i]])
    loss = loss / N 

    grad_x = prob.copy()

    for i in range(N): 
      # go to row i and column of true label y[i] and subtract it by 1 
      grad_x[i,y[i]]  -= 1 
    grad_x = grad_x / N 
